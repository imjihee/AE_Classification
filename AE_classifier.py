...
if __name__ == '__main__':
    ...
    FILE = "param_final.pth" #file to load

    state_dict=torch.load(FILE) #type of state_dict: OrderedDict
    state_dict_enc = OrderedDict()

    #slice state_dict (preprocessing)
    for idx, key in enumerate(state_dict):
        if 0<=idx<4:
            state_dict_enc[key] = state_dict[key]
    
    new_state_dict = OrderedDict()
    for n,v in state_dict_enc.items():
        name = n.replace("encoder.","")
        new_state_dict[name]=v

    #initialize parameter of the each part
    ...
    #fix parameters of encoder model
    ...
    acc.to_excel(excel_writer='training_acc_AEC_100ep_CF100.xlsx')
    lss=pd.DataFrame(train_losses)
    lss.to_excel(excel_writer='training_loss_AEC_100ep_CF100.xlsx')
